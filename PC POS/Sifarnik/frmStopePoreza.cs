...
{
    public partial class frmStopePoreza : Form
    {
        public frmStopePoreza()
        {
            InitializeComponent();
        }
        ...
        private void btnNovo_Click(object sender, EventArgs e)
        {
            ...
            provjera_sql(classSQL.insert(string.Format(@"INSERT INTO porezi
(
    naziv, iznos
)
VALUES
...
txtIznos.Text)));
        ...
        private void dgvSk_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            if (dgvSk.CurrentCell.ColumnIndex == 0)
            {
                try
                {
                    string sql = string.Format(@"UPDATE porezi
SET
    naziv = '{0}'
WHERE id_porez = '{1}';",
dgvSk.Rows[e.RowIndex].Cells["naziv"].FormattedValue.ToString(),
dgvSk.Rows[e.RowIndex].Cells["id_porez"].FormattedValue.ToString());
...
SET
    iznos = '{0}'
WHERE id_porez = '{1}';",
dgvSk.Rows[e.RowIndex].Cells["iznos"].FormattedValue.ToString(),
dgvSk.Rows[e.RowIndex].Cells["id_porez"].FormattedValue.ToString());
