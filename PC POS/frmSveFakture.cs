...
namespace PCPOS
{
    public partial class frmSveFakture : Form
    {
        ...
        private void frmSveFakture_Load(object sender, EventArgs e)
        {
            ...
            dtpOD.Value = new DateTime(currentDate.Year, currentDate.Month, currentDate.Day, 0, 0, 0);
            dtpDO.Value = new DateTime(currentDate.Year, currentDate.Month, currentDate.Day, 23, 59, 59);
        ...
        private void fillDataGrid()
        {
            ...
            */
            ...
            sql = string.Format(@"SELECT {0} fakture.broj_fakture AS [broj], id_ducan, id_kasa,
fakture.id_vd AS [VD], fakture.broj_ispisa, fakture.date AS [Datum], fakture.datum_valute as [Datum valute], valute.ime_valute as [Ime Valute],
id_nacin_placanja, partners.id_partner as sifra_partnera, partners.ime_tvrtke AS [Partner], fakture.ukupno as [Ukupno], fakture.storno as [Storno],
partners.id_partner, (SELECT SUM(uplaceno)
FROM salda_konti WHERE salda_konti.broj_dokumenta = fakture.broj_fakture AND salda_konti.id_ducan = fakture.id_ducan AND salda_konti.id_kasa = fakture.id_kasa AND salda_konti.godina = CAST(fakture.godina_fakture AS INT)) as salda_konti_uplaceno
FROM fakture
INNER JOIN valute ON fakture.id_valuta = valute.id_valuta
...
{1}", top, remote);
        ...
        private void pictureBox1_Click(object sender, EventArgs e)
        {
            ...
            if (chbBroj.Checked)
            {
                Broj = string.Format("fakture.broj_fakture = '{0}' AND ", txtBroj.Text);
            }
            ...
            if (txtPartner.Text.Trim() != "")
            {
                ...
                if (isNum)
                {
                    Partner = string.Format("fakture.id_fakturirati = '{0}' AND ", Str);
                }
                else
                {
                    ...
                    {
                        Partner = string.Format("fakture.id_fakturirati = '{0}' AND ", DSpar.Rows[0][0].ToString());
                    }
                    else
                    {
                        DSpar = classSQL.select(string.Format("SELECT id_partner FROM partners WHERE LOWER(ime_tvrtke) like '%{0}%';", Str), "partners").Tables[0];
                        if (DSpar.Rows.Count > 0)
                        {
                            Partner = string.Format("fakture.id_fakturirati = '{0}' AND ", DSpar.Rows[0][0].ToString());
                        }
            ...
            {
                Poslovnica = string.Format("fakture.id_ducan >= '{0}' AND ", cbPoslovnica.SelectedValue.ToString());
            }

            if (chbOD.Checked)
            {
                DateStart = string.Format("fakture.date >= '{0}' AND ", dtpOD.Value.ToString("yyyy-MM-dd HH:mm:ss"));
            }
            if (chbDO.Checked)
            {
                DateEnd = string.Format("fakture.date <= '{0}' AND ", dtpDO.Value.ToString("yyyy-MM-dd HH:mm:ss"));
            }
            if (chbValuta.Checked)
            {
                Valuta = string.Format("fakture.id_valuta = '{0}' AND ", cbValuta.SelectedValue.ToString());
            }
            if (chbArtikl.Checked)
            {
                SifraArtikla = string.Format("faktura_stavke.sifra = '{0}' AND ", cbArtikl.Text);
            }
            if (chbIzradio.Checked)
            {
                Izradio = string.Format("fakture.id_zaposlenik_izradio = '{0}' AND ", cbIzradio.SelectedValue);
            }
            if (chbNapomenackb.Checked)
            {
                Napomena = string.Format("fakture.napomena ~* '{0}' AND ", textNapomena.Text);
            }
...
WHERE broj_fakture = '{0}' AND fakture.id_valuta = valute.id_valuta AND fakture.id_ducan = '{1}' AND fakture.id_kasa = '{2}';", broj, id_ducan, id_kasa), "fakture").Tables[0];
        ...
        private void printaj_njemacku_formu()
        {
            bool tecaj = true;
            ...
            DataTable DTponude = classSQL.select(string.Format(@"SELECT ime_valute
FROM fakture,valute
WHERE broj_fakture = '{0}' AND fakture.id_valuta=valute.id_valuta;", broj), "fakture").Tables[0];
            ...
            Report.Faktura_njem.FakturaNjemackiReportForm fakturaNjemackiReport = new Report.Faktura_njem.FakturaNjemackiReportForm();
            fakturaNjemackiReport.dokumenat = "FAK";
            fakturaNjemackiReport.racunajTecaj = tecaj;
            fakturaNjemackiReport.ImeForme = "Fakture";
            fakturaNjemackiReport.poslovnica = id_ducan;
            fakturaNjemackiReport.naplatni = id_kasa;
            fakturaNjemackiReport.broj_dokumenta = dgv.CurrentRow.Cells["broj"].FormattedValue.ToString();
            fakturaNjemackiReport.ShowDialog();
        }
            ...
            if (e.KeyCode == Keys.Enter)
            {
                ...
                if (txtPartner.Text == "")
                {
                    ...
                    if (Properties.Settings.Default.id_partner != "")
                    {
                        ...
                        partner = classSQL.select(string.Format(@"SELECT id_partner, ime_tvrtke
FROM partners
WHERE id_partner = '{0}';", Properties.Settings.Default.id_partner), "partners");
                ...
                {
                    DataTable DSpar = classSQL.select("SELECT ime_tvrtke, id_partner FROM partners WHERE LOWER(ime_tvrtke) = '" +
                        Str + "'", "partners").Tables[0];
                    if (DSpar.Rows.Count > 0)
                    {
                        txtPartner.Text = DSpar.Rows[0]["ime_tvrtke"].ToString();
                        Properties.Settings.Default.id_partner = DSpar.Rows[0]["id_partner"].ToString();
                        System.Windows.Forms.SendKeys.Send("{TAB}");
                    }
                    else
                    {
                        DSpar = classSQL.select("SELECT ime_tvrtke, id_partner FROM partners WHERE LOWER(ime_tvrtke) like '%" +
                            Str + "%'", "partners").Tables[0];
                        if (DSpar.Rows.Count > 0)
                        {
                            txtPartner.Text = DSpar.Rows[0]["ime_tvrtke"].ToString();
