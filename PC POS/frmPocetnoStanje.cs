...
namespace PCPOS
{
    public partial class frmPocetnoStanje : Form
    {
        ...
        private void SetRoba()
        {
            ...
            dgw.Rows[br].Cells["EAN"].Value = DTRoba.Rows[0]["ean"].ToString();
            ...
            dgw.Rows[br].Cells["pdv"].Value = DTRoba.Rows[0]["porez"].ToString();
            dgw.Rows[br].Cells["mpc"].Value = Math.Round(
                ((Convert.ToDouble(DTRoba.Rows[0]["vpc"].ToString()) * Convert.ToDouble(DTRoba.Rows[0]["porez"].ToString())) / 100)
                + Convert.ToDouble(DTRoba.Rows[0]["vpc"].ToString()), 2
                ).ToString("#0.00");
        ...
        private void dgw_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            else if (dgw.CurrentCell.ColumnIndex == 9)
            {
                try
                {
                    ...
                    dgw.CurrentRow.Cells["vpc"].Value = Convert.ToDouble(MPC / (1 + Convert.ToDouble(dgw.CurrentRow.Cells["pdv"].FormattedValue.ToString()))).ToString("#0.00");
        ...
        private void Spremi()
        {
            ...
            for (int i = 0; i < dgw.Rows.Count; i++)
            {
                ...
                pdvPDV = dgw.Rows[i].Cells["pdv"].FormattedValue.ToString();
        ...
        public void GetAmount(string sifra, string skladiste, string kolicina, string vpc, string mpc, string ean, string nc)
        {
            ...
            else
            {
                string sql = "UPDATE roba_prodaja SET kolicina='" + kolicina + "'," +
                     "nc='" + nc + "'," +
                     "vpc='" + vpc.Replace(",", ".") + "'," +
                     "porez='" + pdvPDV + "' WHERE id_skladiste='" + skladiste + "' AND sifra='" + sifra + "'";
                provjera_sql(classSQL.update(sql));
        ...
        private void FillPocetnoStanje(string broj)
        {
            ...
            for (int i = 0; i < DT.Rows.Count; i++)
            {
                ...
                dgw.Rows[br].Cells["EAN"].Value = DT.Rows[i]["ean"].ToString();
                dgw.Rows[br].Cells["jmj"].Value = DT.Rows[i]["jm"].ToString();
                dgw.Rows[br].Cells["kolicina"].Value = DT.Rows[i]["kolicina"].ToString();
                dgw.Rows[br].Cells["nbc"].Value = DT.Rows[i]["nbc"].ToString();
                dgw.Rows[br].Cells["mpc"].Value = Convert.ToDouble(DT.Rows[i]["mpc"].ToString()).ToString("#0.000");
                dgw.Rows[br].Cells["vpc"].Value = Convert.ToDouble(Convert.ToDouble(DT.Rows[i]["mpc"].ToString()) / (1 + Convert.ToDouble(DT.Rows[i]["pdv"].ToString()) / 100)).ToString("#0.000");
        ...
        private void btnObrisi_Click(object sender, EventArgs e)
        {
            ...
            if (dgw.Rows[dgw.CurrentRow.Index].Cells["id_stavka"].Value != null)
            {
                ...
                {
                    classSQL.delete("DELETE FROM pocetno_stanje_stavke WHERE id_stavka='" + dgw.Rows[dgw.CurrentRow.Index].Cells["id_stavka"].FormattedValue.ToString() + "'");
                    dgw.Rows.RemoveAt(dgw.CurrentRow.Index);
                    classSQL.insert("INSERT INTO aktivnost_zaposlenici (id_zaposlenik,datum,radnja) VALUES ('" + Properties.Settings.Default.id_zaposlenik + "','" + DateTime.Now.ToString("yyyy-MM-dd H:mm:ss") + "','Brisanje stavke s početnog stanja br." + txtBrojInventure.Text + "')");
