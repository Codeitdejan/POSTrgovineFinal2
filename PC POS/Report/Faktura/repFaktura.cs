...
namespace PCPOS.Report.Faktura
{
    public partial class repFaktura : Form
    {
        ...
        private ReportParameter p10, p21;
        ...
        private void FillRacun(string broj, string[] imeTablica, string poslovnica, string naplatni)
        {
            ...
            if (imeTablica[0] == "racuni")
            {
                ...
                //                if (imeTablica[0] == "racuni")
                //                {
                string sql_avio = string.Format(@"select avio_registracija, avio_tip_zrakoplova, avio_maks_tezina_polijetanja
from {0}
...
imeTablica[0], broj, poslovnica, naplatni);
        ...
        private void FillFaktura(string broj, string[] imeTablica)
        {
            ...
            if (DT1.Rows[0]["broj_avansa"].ToString() != "0" && DT1.Rows[0]["broj_avansa"].ToString() != "")
            {
                ...
                avans_porezi_novo += "\r\n" + string.Format("{0:n}", Math.Round(aUku, 2, MidpointRounding.AwayFromZero)) + " kn\r\n";
                avans_porezi_novo += string.Format("{0:n}", Math.Round(aOnov, 2, MidpointRounding.AwayFromZero)) + " kn\r\n";
                avans_porezi_novo += string.Format("{0:n}", Math.Round(aPdv, 2, MidpointRounding.AwayFromZero)) + " kn\r\n";
                avans_porezi_novo += string.Format("{0:n}", Math.Round(razlika_pdva, 3, MidpointRounding.AwayFromZero)) + " kn";
                ...
                avans_razlika_zaPlatiti = "Po ovom računu zbog obračuna predujma razlika za plačanje je:  " + string.Format("{0:n}", (Math.Round(Convert.ToDecimal(sveUkupno), 2, MidpointRounding.AwayFromZero) - aUku)) + " kn\r\n";
            }
