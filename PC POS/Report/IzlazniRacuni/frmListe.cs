...
DTheader.Rows[i]["broj_avansa"].ToString(), DTheader.Rows[i]["poslovnica"].ToString());
        ...
        private void ListaFakture()
        {
            ...
            for (int i = 0; i < DTheader.Rows.Count; i++)
            {
                ...
                decimal vpc2 = 0;
                decimal usl_bez_por = 0;
                decimal usl_bez_por_rabat = 0;
                decimal roba_bez_por = 0;
                decimal roba_bez_por_rabat = 0;
                ...
                for (int y = 0; y < DTstavke.Rows.Count; y++)
                {
                    ...
                    if (DTstavke.Rows[y]["oduzmi"].ToString() != "DA")
                    {
                        ...
                        usl_bez_por_rabat += usluga_stavke - ((vpc1 * (rabat / 100)) * kol);
                    }
                    else
                    {
                        roba_stavke = (vpc1 * kol);
                        ...
                        roba_bez_por_rabat += roba_stavke - ((vpc1 * (rabat / 100)) * kol);
                    }

                    rabat_proracun = ((vpc1 * (rabat / 100)) * kol) + rabat_proracun;
                    decimal vpcSRabatom = vpc1 - (vpc1 * (rabat / 100));
                    if (vpcSRabatom == 0) vpcSRabatom = vpc1;
                    iznos_neto = (vpc1 * kol) + iznos_neto;
                    ...
                    if (kol != 0)
                    {
                        ...
                        if (stope.Length == 0 || stope2.Length == 0)
                        {
                            ...
                            stopa2["osnovica"] = Math.Round((vpcSRabatom * kol), 6, MidpointRounding.AwayFromZero);
                        ...
                        else
                        {
                            ...
                            stope2[0]["osnovica"] = (osnovicaOld2 + Math.Round((vpcSRabatom * kol), 6, MidpointRounding.AwayFromZero));
