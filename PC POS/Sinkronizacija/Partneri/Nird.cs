...
{
    public partial class Nird : Form
    {
        public Nird()
        {
            InitializeComponent();
        ...
        {
            DT.Rows[row]["Grupa"] = "Nird";
            if (webBrowser1.Url != curPage)
            {
                curPage = webBrowser1.Url;
                int x, y;
                x = webBrowser1.DocumentText.IndexOf("slikadetail");
                if (x != -1)
                {
                    x = webBrowser1.DocumentText.IndexOf("href", x) + 6;
                    y = webBrowser1.DocumentText.IndexOf("\"", x);
                    string slikaLink = webBrowser1.DocumentText.Substring(x, y - x);
                    DT.Rows[row]["LinkSlikeWeb"] = "http://proizvodi.nird.hr/" + slikaLink;
                }

                x = webBrowser1.DocumentText.IndexOf("opisdetail0");
                if (x != -1)
                {
                    x = webBrowser1.DocumentText.IndexOf("<h1>", x) + 4;
                    y = webBrowser1.DocumentText.IndexOf("</", x);
                    string ime = webBrowser1.DocumentText.Substring(x, y - x);
                    DT.Rows[row]["Naziv"] = ime;

                    x = webBrowser1.DocumentText.IndexOf("Uzrast:", x) + 13;
                    y = webBrowser1.DocumentText.IndexOf("</", x);
                    string uzrast = webBrowser1.DocumentText.Substring(x, y - x);
                    x = webBrowser1.DocumentText.IndexOf("OPIS1", x) + 7;
                    y = webBrowser1.DocumentText.IndexOf("</", x);
                    string opis = webBrowser1.DocumentText.Substring(x, y - x);

                    DT.Rows[row]["Opis"] = opis + " Uzrast: " + uzrast;
                }

                x = webBrowser1.DocumentText.IndexOf("linklista");
                if (x != -1)
                {
                    x = webBrowser1.DocumentText.IndexOf("linklista", x + 1) + 1;
                    x = webBrowser1.DocumentText.IndexOf("h2", x + 1) + 1;
                    x = webBrowser1.DocumentText.IndexOf(">", x) + 1;
                    y = webBrowser1.DocumentText.IndexOf("</", x);
                    string kate = webBrowser1.DocumentText.Substring(x, y - x);
                    DT.Rows[row]["Podgrupa"] = kate;
                }

                progressBar1.Value = (row * 100) / DT.Rows.Count;
                row++;
                if (row == DT.Rows.Count) { row = 0; browserComplete(); return; }
                string url = "http://proizvodi.nird.hr/-" + DT.Rows[row]["Sifra"].ToString();
                webBrowser1.Navigate(new Uri(url));
            }
        }
