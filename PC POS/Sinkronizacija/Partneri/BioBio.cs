...
{
    public partial class frmBioBio : Form
    {
        public frmBioBio()
        {
            InitializeComponent();
        ...
        private string GetSlike(string naziv, string sifra)
        {
            sifra = sifra.Remove(0, 3);
            naziv = naziv.Replace(" ", "-") + "-" + sifra;

            string urlAddress = "http://www.biobio.hr/" + naziv.ToLower();
            bool postoji = false, postoji_src = false;
            string str = GetHtml(urlAddress), slika = "";

            for (int i = 0; i < str.Length; i++)
            {
                if (str[i] == 'p' && str[i + 3] == 'd' && str[i + 6] == 't' && str[i + 8] == 'i' && str[i + 12] == 'e' && str[i + 16] == 'g')
                {
                    postoji = true;
                }
                else if (postoji)
                {
                    if (postoji && str[i] == '\'' && str[i - 1] == '=' && str[i - 4] == 's')
                    {
                        postoji_src = true;
                    }
                    else if (postoji_src)
            ...
            return "http://www.biobio.hr" + slika;
        }
        ...
        private void loadXLS(string path)
        {
            ...
            for (int i = 0; i < DT.Rows.Count; i++)
            {
                ...
                try
                {
                    slika = GetSlike(DT.Rows[i]["naziv"].ToString(), DT.Rows[i]["sifra"].ToString());
                }
                catch
                { }
