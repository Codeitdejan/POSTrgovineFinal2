...
namespace PCPOS.Kasa
{
    public partial class frmZavrsiSmjenu : Form
    {
        ...
        private void LoadData()
        {
            ...
            string sql = "SELECT * FROM smjene WHERE id='" + zadnjiID + "'";
            ...
            DataTable DT_zaposlenik1 = classSQL.select("SELECT ime + ' ' + prezime FROM zaposlenici WHERE id_zaposlenik='" + DT_smjena.Rows[0]["blagajnik"].ToString() + "'", "zaposlenici").Tables[0];
            ...
            string sql1 = "SELECT SUM(ukupno_gotovina-(ukupno_karticezbrojukupno_gotovinazbrojukupno_bon-ukupno)) AS [gotovina],SUM(ukupno_kartice) AS [kartice],SUM(ukupno_virman) AS [virman], SUM(ukupno_bon) AS [bon] FROM racuni " +
            ...
            if (DT.Rows.Count > 0 && DT.Rows[0][0].ToString() != "")
            {
                ...
                {
                    if (DT.Rows[0]["bon"].ToString() != "") { lblProdanoVirman.Text = "Ukupno prodano u smjeni bon: " + string.Format("{0:0.00}", Convert.ToDouble(DT.Rows[0]["bon"].ToString())) + " kn"; } else { lblProdanoVirman.Text = "Bon: 0.00 kn"; }
                }
                else
                {
            ...
            DataTable DTblagajnici = classSQL.select(sqlBlagajnik, "zaposlenici").Tables[0];
            flpArtikli.Controls.Clear();
            int i = 1;

            foreach (DataRow row in DTblagajnici.Rows)
            {
                ...
                lbl.Font = new System.Drawing.Font("Verdana", 10.25F, System.Drawing.FontStyle.Regular, System.Drawing.GraphicsUnit.Point, 0);
                lbl.Location = new System.Drawing.Point(10, 15);
                lbl.AutoSize = true;
                lbl.TabIndex = i;

                string imeBlagajnika = row["ime"].ToString() + " " + row["prezime"].ToString();
                ...
                if (Class.PodaciTvrtka.oibTvrtke == "40097758416")
                {
                    lbl.Text += " \n\rBon: " + Convert.ToDouble(row["bon"].ToString()).ToString("#0.00") + " kn";
                }
                else
                {
        ...
        private void btnZavrsiSmjenu_Click(object sender, EventArgs e)
        {
            ...
            string sql = "UPDATE smjene SET zavrsetak='" + DateTime.Now.ToString("yyyy-MM-dd H:mm:ss") + "', zavrsno_stanje='" + Sukupno.Replace(",", ".") + "',blagajnikz='" + Properties.Settings.Default.id_zaposlenik + "' WHERE id='" + zadnjiID + "'";
        ...
        private void PrometProdajneRobe(bool stavke)
        {
            ...
            string sql = "SELECT " +
                ...
                " ROUND(CAST(racun_stavke.mpc as numeric) - ( CAST(racun_stavke.mpc as numeric) * ( CAST(REPLACE(racun_stavke.rabat,',','.') as numeric) / 100 )),2) as mpc," +
            ...
            PrintTextLine("Ukupno komada:    " + ukupnoKol.ToString("#0.000"));
