...
namespace PCPOS
{
    ...
    {
        public bool OduzmiStavke { get; set; }
        public frmOtpremnica(bool oduzmiStavke = true)
        {
            OduzmiStavke = oduzmiStavke;
            InitializeComponent();
            //this.Text = "Otpremnica na komisiju";
            this.chbOtpremnicaZaKomisiju.Checked = !OduzmiStavke;
            if (Class.PodaciTvrtka.oibTvrtke == "88985647471") {
                this.chbOtpremnicaZaKomisiju.Checked = true;
            }
            //this.chbBezPdVa.Checked = true;
            //this.chbSNBC.Checked = false;
        }
        ...
        {
            string sql = "SELECT MAX(broj_otpremnice) FROM otpremnice WHERE id_skladiste='" + cbSkladiste.SelectedValue + "'";
        ...
        private void SetRoba()
        {
            ...
            if (!OduzmiStavke) {
                dgw.Rows[br].Cells["oduzmi"].Value = "NE".ToString();
            }
        ...
        private void izracun()
        {
            ...
            System.Globalization.NumberStyles.AllowLeadingWhite | System.Globalization.NumberStyles.AllowTrailingWhite) == false) { dgw.Rows[rowBR].Cells["kolicina"].Value = "1"; MessageBox.Show("Greška kod upisa količine.", "Greška"); }
                ...
                {
                    decimal _nbc = Class.FIFO.getNbc(Util.Korisno.GodinaKojaSeKoristiUbazi, dtpDatum.Value, Convert.ToInt32(cbSkladiste.SelectedValue.ToString()), kol, dgw.Rows[rowBR].Cells["sifra"].Value.ToString(), nbc);
                ...
                if (chbSNBC.Checked && Class.Postavke.proizvodnjaFakturaNbc)
                {
                    nbc = 0;
                    nbc = Math.Round(Convert.ToDouble(dgw.Rows[rowBR].Cells["nc"].FormattedValue.ToString()), 2);
                    vpc = nbc;
                    //dgw.Rows[rowBR].Cells["vpc"].Value = Math.Round(vpc, 2).ToString("#0.00");
                }
                ...
                //za nbc otpremnicu
                nbc = 0;
                double nbcUkupno = 0, nbcPdv = 0, nbcSveUkupno = 0;
                nbc = Math.Round(Convert.ToDouble(dgw.Rows[rowBR].Cells["nc"].FormattedValue.ToString()), 2);
                nbcUkupno = nbc * kol;
        ...
        private void txtSifra_robe_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                string sql = "SELECT * FROM roba WHERE sifra='" + sifra + "'";
        ...
        private void printaj(string broj, string skl)
        {
            ...
            {
                Report.Faktura.repFakturaNovo pr = new Report.Faktura.repFakturaNovo();
                pr.dokumenat = "OTP";
                pr.broj_dokumenta = broj;
                pr.from_skladiste = skl;
                pr.ImeForme = "Otpremnica";
                pr.id_partner_poslovnica = Convert.ToInt32(cmbPoslovnicaPartner.SelectedValue);
                pr.ShowDialog();
            }
        ...
        private void btnSpremi_Click(object sender, EventArgs e)
        {
            ...
            {
                sql = "SELECT * FROM otpremnice WHERE broj_otpremnice = '" + txtBrojOtpremnice.Text +
                    "' AND id_skladiste='" + cbSkladiste.SelectedValue.ToString() + "' AND godina_otpremnice='" + nmGodinaOtpremnice.Value + "'";
                //fill header
            ...
            sql = "INSERT INTO otpremnice (broj_otpremnice,partner_osoba,godina_otpremnice,id_skladiste,osoba_partner,id_odrediste," +
                ...
                "registracija,istovarno_mj,istovarni_rok,troskovi_prijevoza,ukupno, novo, editirano, use_nbc, partner_poslovnica, oduzmi_iz_skladista) VALUES (" +
            ...
            provjera_sql(classSQL.insert("INSERT INTO aktivnost_zaposlenici (id_zaposlenik,datum,radnja) VALUES ('" + Properties.Settings.Default.id_zaposlenik + "','" + DateTime.Now.ToString("yyyy-MM-dd H:mm:ss") + "','Nova otpremnica br." + txtBrojOtpremnice.Text + "')"));
        ...
        private void btnNoviUnos_Click(object sender, EventArgs e)
        {
            ...
            //chbOduzmiStavke.Enabled = false;
            EnableHeader(false);
            txtSifraOdrediste.Select();
            ControlDisableEnable(0, 1, 1, 0, 0);
        }

        private void EnableHeader(bool enable = true) {
            txtBrojOtpremnice.Enabled = enable;
            nmGodinaOtpremnice.Enabled = enable;
            cbSkladiste.Enabled = enable;
            chbOtpremnicaZaKomisiju.Enabled = enable;
        }
        private void btnOdustani_Click(object sender, EventArgs e)
        {
            ...
            //nmGodinaOtpremnice.ReadOnly = false;
            //txtBrojOtpremnice.Enabled = true;
            //nmGodinaOtpremnice.Enabled = true;
            //cbSkladiste.Enabled = true;
            //chbOduzmiStavke.Enabled = true;
            EnableHeader(true);
            //txtBrojOtpremnice.Enabled = true;
            ControlDisableEnable(1, 0, 0, 1, 0);
        }
        ...
        private void fillOtpremnice(string skl, string godina)
        {
            ...
            chbOtpremnicaZaKomisiju.Checked = !Convert.ToBoolean(DTotpremnice.Rows[0]["oduzmi_iz_skladista"].ToString());
            getPoslovnicaPartner();
            cmbPoslovnicaPartner.SelectedValue = DTotpremnice.Rows[0]["partner_poslovnica"];
        ...
        private void btnDeleteAllFaktura_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Brisanjem ove otpremnice brišete i količinu robe sa skladišta. Da li ste sigurni da želite obrisati ovu otpremnicu?", "Upozorenje", MessageBoxButtons.YesNo, MessageBoxIcon.Information) == DialogResult.Yes)
            {
                ...
                classSQL.insert("INSERT INTO aktivnost_zaposlenici (id_zaposlenik,datum,radnja) VALUES ('" + Properties.Settings.Default.id_zaposlenik + "','" + DateTime.Now.ToString("yyyy-MM-dd H:mm:ss") + "','Brisanje cijele otpremnice br." + txtBrojOtpremnice.Text + "')");
        ...
        private void UpdateOtpremnica()
        {
            ...
            string sql = "UPDATE otpremnice SET " +
                ...
                " oduzmi_iz_skladista='" + (OduzmiStavke ? 1 : 0) + "'" +
        ...
        private void txtBrojOtpremnice_KeyDown_1(object sender, KeyEventArgs e)
        {
            ...
            if (e.KeyCode == Keys.Enter)
            {
                chbOtpremnicaZaKomisiju.Checked = false;
...
where id_partner = {0}", txtSifraOdrediste.Text.Trim());
        ...
        private void chbOtpremnicaZaKomisiju_CheckedChanged(object sender, EventArgs e)
        {
            OduzmiStavke = !chbOtpremnicaZaKomisiju.Checked;
        }
    }
}