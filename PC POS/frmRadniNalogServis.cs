...
namespace PCPOS
{
    public partial class frmRadniNalogSerivs : Form
    {
        public string broj_rn_edit { get; set; }

        public frmRadniNalogSerivs()
        {
            InitializeComponent();
        }
        ...
        private void btnSpremi_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < dgw.Rows.Count; i++)
            {
                ...
                {
                    MessageBox.Show("Servis nije spremljen zbog ne odabira skladišta na pojedinim artiklima.", "Greška");
                    return;
                }
            }
            ...
            Util.AktivnostZaposlenika.SpremiAktivnost(dgw, null, "Radni nalog servis", ttxBrojRN.Text, false);
        ...
        private void UpdateRN()
        {
            ...
            Util.AktivnostZaposlenika.SpremiAktivnost(dgw, null, "Radni nalog servis", ttxBrojRN.Text, true);
            edit = false;
            EnableDisable(false);
            deleteFields();
            btnSpremi.Enabled = false;
            ...
            if (dgw.Rows[dgw.CurrentRow.Index].Cells["id_stavka"].Value != null)
            {
                if (MessageBox.Show("Da li ste sigurni da želite obrisati ovu stavku?", "Upozorenje", MessageBoxButtons.YesNo, MessageBoxIcon.Information) == DialogResult.Yes)
                {
                    ...
                    classSQL.insert("INSERT INTO aktivnost_zaposlenici (id_zaposlenik,datum,radnja)" +
                        "VALUES ('" + Properties.Settings.Default.id_zaposlenik + "','" + DateTime.Now.ToString("yyyy-MM-dd H:mm:ss") + "','Brisanje stavke sa RN servisa br." + ttxBrojRN.Text + "')");
                    Util.AktivnostZaposlenika.SpremiAktivnost(dgw, null, "Radni nalog servis", ttxBrojRN.Text, true);
        ...
        private void btnDeleteAllFaktura_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Da li ste sigurni da želite obrisati ovaj servis?", "Upozorenje", MessageBoxButtons.YesNo, MessageBoxIcon.Information) == DialogResult.Yes)
            {
                ...
                Util.AktivnostZaposlenika.SpremiAktivnost(new DataGridView(), null, "Radni nalog servis", ttxBrojRN.Text, true);
                MessageBox.Show("Obrisano.");
        ...
        private void dgw_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            else if (dgw.CurrentCell.ColumnIndex == 7)
            {
                try
                {
                    dgw.CurrentRow.Cells["vpc"].Value = Convert.ToDouble(dgw.CurrentRow.Cells["mpc"].FormattedValue.ToString()) / Convert.ToDouble(1 + Convert.ToDouble(dgw.CurrentRow.Cells["porez"].FormattedValue.ToString()) / 100);
                }
                catch (Exception ex)
                {
